-- Social feed completo (tipo Instagram) para SUMO
-- Incluye: posts, media (foto/video), likes, comentarios, menciones, funciones feed/perfil
-- e integración de Storage bucket `files` con estructura por carpetas.
...
create index if not exists post_media_post_sort_idx on public.post_media (post_id, sort_order asc);
...
-- ---------------------------------------------------------------------
-- 6) Trigger updated_at (si existe set_updated_at)
-- ---------------------------------------------------------------------
do $$
begin
...
-- Lectura pública (solo posts visibility='public')
...
using (exists (select 1 from public.posts p where p.id = post_media.post_id and p.visibility = 'public'));

drop policy if exists post_likes_select_public on public.post_likes;
create policy post_likes_select_public
on public.post_likes for select
using (exists (select 1 from public.posts p where p.id = post_likes.post_id and p.visibility = 'public'));

drop policy if exists post_comments_select_public on public.post_comments;
create policy post_comments_select_public
on public.post_comments for select
using (exists (select 1 from public.posts p where p.id = post_comments.post_id and p.visibility = 'public'));
...
using (exists (select 1 from public.posts p where p.id = post_mentions.post_id and p.visibility = 'public'));

-- Escritura: dueños y usuarios autenticados
...
drop policy if exists post_media_insert_post_owner on public.post_media;
...
drop policy if exists post_media_delete_post_owner on public.post_media;
...
with check (exists (select 1 from public.posts p where p.id = post_mentions.post_id and p.author_id = auth.uid()));
...
using (exists (select 1 from public.posts p where p.id = post_mentions.post_id and p.author_id = auth.uid()));

-- ---------------------------------------------------------------------
-- 8) Funciones útiles de lectura
-- ---------------------------------------------------------------------
...
create or replace function public.profile_posts(profile_uuid uuid, limit_count int default 20, offset_count int default 0)
returns table (
  id uuid,
  author_id uuid,
  body text,
...
-- ---------------------------------------------------------------------
-- 9) Storage (bucket files + policies)
-- Estructura esperada del backend:
-- sumo/{bikes|routes|profiles|posts|misc}/.../{user_id}/{yyyy}/{mm}/{dd}/{file}
-- ---------------------------------------------------------------------
...
using (bucket_id = 'files' and split_part(name, '/', 1) = 'sumo');

drop policy if exists files_auth_insert_own on storage.objects;
create policy files_auth_insert_own
on storage.objects for insert
with check (
  bucket_id = 'files'
  and auth.role() = 'authenticated'
  and split_part(name, '/', 1) = 'sumo'
  and split_part(name, '/', 4) = auth.uid()::text
);

drop policy if exists files_auth_update_own on storage.objects;
create policy files_auth_update_own
...
drop policy if exists files_auth_delete_own on storage.objects;
create policy files_auth_delete_own
